...
The pot insert interior is always treated as a grid. The default `Grid_Rows = 1` and `Grid_Columns = 1` creates one open compartment, while larger values add solid internal divider walls. `Grid_Row_Sizes` and `Grid_Column_Sizes` are comma-separated strings that control compartment sizing with values such as `1*`, `2*`, `1`, or `25%`. `Grid_Cell_Roles` is a comma-separated row-major role string where each cell can be `Pot`, `Box`, or `FillTube`; `Pot` cells get drain holes, `Box` cells stay closed at the bottom, and `FillTube` cells get a larger bottom opening controlled by `Fill_Tube_Diameter`. `Grid_Wall_Thickness` controls the divider wall thickness.
