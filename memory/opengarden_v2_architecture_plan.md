...
- Container support deck is a continuous reservoir-level deck (`Container_Support_Deck_Mode = "None"`, `"Rails"`, or `"Grid"`). `Rails` emits long rails only; `Grid` adds flat-top arch bridges between rails. Keepouts are subtracted for components whose `support_mode` is not `"deck"`.
